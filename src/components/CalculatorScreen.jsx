...
const CalculatorScreen = ({ result, screenInput }) => {
  ...
  return (
    <div  className=" w-1/3 h-1/5 bg-teallight  text-4xl text-tealdark">
      ...
      <p data-testid="screen-input" className="text-2xl text-end">{screenInput}</p>
        {result != 0 ? <p data-testid="screen-result">{parseFloat((result).toFixed(5))}</p> : <p data-testid="screen-result"> 0</p>}
      </div>
    </div>
  );
};
